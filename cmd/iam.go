...
func (sys *IAMSys) IsAllowed(args iampolicy.Args) bool {
	...
	// As policy is not available for given bucket name, returns false.
	return false
}
...
func (sys *IAMSys) refresh(objAPI ObjectLayer) error {
	...
	// Admin credentials by default allowed to perform all S3 operations.
	{
		adminKey := globalServerConfig.GetCredential().AccessKey
		_, ok := iamPolicyMap[adminKey]
		if !ok {
			iamPolicyMap[adminKey] = iampolicy.DefaultAllowAll
		}
	}
